    ...
    <n-form ref="formRef" :model="model" :rules="rules" :size="size" label-placement="top">
      <n-grid :span="24" :x-gap="24">
        ...
        <n-form-item-gi :span="12" label="Status" path="status">
          <n-select v-model:value="model.status" :options="status" />
        </n-form-item-gi>
        ...
        <n-form-item-gi :span="12" label="Creation Date" path="creationDate">
          <n-date-picker
            v-model:value="model.creationDate"
            type="datetime"
        ...
        <n-form-item-gi :span="12" label="Closing Date" path="closingDate">
          <n-date-picker
            v-model:value="model.closingDate"
            type="datetime"
            placeholder="Enter date"
          />
        </n-form-item-gi>
...
const model = ref({
  ...
  status:
    props.manga != undefined && props.isEdit ? props.manga.status : (StatusType.ONGOING as string),
  ...
  creationDate:
    props.manga != undefined && props.isEdit ? props.manga.creationDate : (null as Date | null),
  closingDate:
    props.manga != undefined && props.isEdit ? props.manga.endDate : (null as Date | null)
})

const status = [StatusType.ONGOING, StatusType.FINISHED].map((v) => ({
  label: v,
  value: v
}))
